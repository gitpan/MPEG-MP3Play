...
convert_message_to_HV ( XA_Message* message ) {
	...
	switch (message->code) {
		...
		case XA_MSG_NOTIFY_INPUT_STREAM_INFO:
			...
			hv_store (msg_hash,
				"stream_info_mode", 16,
				newSViv ((IV)message->data.stream_info.mode),
				0);
			hv_store (msg_hash,
				"stream_info_layer", 17,
				newSViv ((IV)message->data.stream_info.layer),
				0);
			hv_store (msg_hash,
				"stream_info_level", 17,
				newSViv ((IV)message->data.stream_info.level),
				0);
			hv_store (msg_hash,
				"stream_info_frequency", 21,
				newSViv ((IV)message->data.stream_info.frequency),
				0);
			break;
